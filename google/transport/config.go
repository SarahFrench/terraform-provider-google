...
func ExpandExternalCredentialsHcpTerraformConfig(v interface{}) (*ExternalCredentialsHcpTerraform, error) {
	...
	config := &ExternalCredentialsHcpTerraform{}
	cfgV := ls[0].(map[string]interface{})

	audience, ok := cfgV["audience"]
	if !ok || audience.(string) == "" {
		return nil, errors.New("missing value for external_credentials_hcp_terraform.audience")
	}
	config.Audience = audience.(string)

	email, ok := cfgV["service_account_email"]
	if !ok || email.(string) == "" {
		return nil, errors.New("missing value for external_credentials_hcp_terraform.service_account_email")
	}
...
type Config struct {
	...
	ExternalCredentialsHcpTerraform           *ExternalCredentialsHcpTerraform
	Project                                   string
	Region                                    string
